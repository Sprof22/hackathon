...
export default function Approvals() {
  ...
  async function load() {
    try {
      const data = await api<Reminder[]>("/approvals");
      setItems(data);
      setMessage(data.length ? "" : "No reminders are waiting for approval.");
    } catch (err) {
  ...
  async function approve(id: string) {
    ...
    setMessage("Approved. The email was sent or captured according to your email settings.");
  }
  return (
    <main className="flow-page">
      <div className="flow-wrap">
        ...
        <div className="flow-heading">
          ...
          <p>Nothing leaves LoopClose until a manager reviews it.</p>
        </div>
        <section className="list-card">
          {items.map((item) => (
            <article className="list-entry" key={item.id}>
              ...
              <button onClick={() => approve(item.id)}>Approve & send</button>
            </article>
          ))}
