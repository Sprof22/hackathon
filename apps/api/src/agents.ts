...
export type ExtractedItem = { task:string; ownerName:string; ownerEmail:string|null; deadline:Date|null; sourceQuote:string };
export type Verdict = { status:ItemStatus; confidence:number|null; evidenceQuote:string|null };
...
export class ExtractionService {
  ...
  async extract(transcript:string, meetingDate:Date):Promise<ExtractedItem[]> {
    if (this.config.get("LLM_PROVIDER") === "ollama") {
      try { return await this.ollama(transcript, meetingDate); } catch { /* deterministic fallback keeps demos runnable */ }
    }
  ...
  private rules(transcript:string, meetingDate:Date) {
    ...
    return lines.flatMap(line => {
      ...
      return [{ task:taskText.replace(/^(finish|complete|handle)\s+/i,""), ownerName:match[1], ownerEmail:null, deadline:date && !Number.isNaN(date.valueOf()) ? date : null, sourceQuote:line }];
    });
  }
  private async ollama(transcript:string, meetingDate:Date):Promise<ExtractedItem[]> {
    const response = await fetch(`${this.config.get("OLLAMA_URL","http://localhost:11434")}/api/chat`, { method:"POST", headers:{"content-type":"application/json"}, body:JSON.stringify({ model:this.config.get("OLLAMA_MODEL","llama3.2"), stream:false, format:"json", messages:[{role:"system",content:"Extract only explicit commitments. Return JSON: {items:[{task,ownerName,ownerEmail,deadline,sourceQuote}]}. Never invent evidence."},{role:"user",content:`Meeting date: ${meetingDate.toISOString()}\n${transcript}`}] }) });
    if (!response.ok) throw new Error("Ollama unavailable");
    const payload = await response.json() as { message:{content:string} };
    const parsed = JSON.parse(payload.message.content) as { items:Array<Record<string,string|null>> };
    return parsed.items.map(v=>({ task:String(v.task), ownerName:String(v.ownerName), ownerEmail:v.ownerEmail, deadline:v.deadline?new Date(v.deadline):null, sourceQuote:String(v.sourceQuote) }));
  }
}
...
export class VerificationService {
  ...
  async verify(meeting:Meeting) {
    ...
    for (const item of open.filter(v=>v.meeting.id !== meeting.id)) {
      ...
      if (!verdict.evidenceQuote) item.silentMeetingCount += 1; else item.silentMeetingCount = 0;
      const threshold = Number(this.config.get("AUTO_CLOSE_THRESHOLD",0.86));
      if (verdict.status === ItemStatus.DONE && verdict.evidenceQuote && (verdict.confidence ?? 0) >= threshold) { item.status=ItemStatus.DONE; item.autoClosed=true; item.resolvedAt=new Date(); }
      ...
      else if (item.silentMeetingCount >= 2 || (item.deadline && item.deadline < meeting.meetingDate)) item.status=ItemStatus.STALE;
